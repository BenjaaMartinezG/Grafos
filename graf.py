...
class Grafo:
    ...
    def dijkstra(self, vert): #Forma 1, modificar, con arista {[vertice1, vertice2, peso], ....}
      n = len(self.vertices)   #El largo del n vertices
      contador = 0
      l = 0  # Distancia acumulada en peso, inicializando con 0
      x = [] #Para almacenar los nodos que se finalizan
      y = [] #Para almacenar aristas en la cual se hará comparaciones
      F = [[0,'-']] #Para almacenar el peso acumulado y su vertice origen
      while(contador < n):
        peso = 10000
        validacion2 = 1 #En caso de que no se ha encontrado ningun vertice y se evita el error de agregar
        for arista in self.aristas:  #Recorre las aristas que contiene el grafo
          if(arista[0] == vert or arista[1] == vert):
            y.append(arista)
        for c in y:
          validacion = 1 #En caso de que el nodo ya estaba finalizado
          if(l+int(c[2]) < l+peso):
            for com in x: #Para comparar con los nodos que estan finalizado
              if(c[0] == com or c[1] == com):
                validacion = 0
            if(validacion == 1):
              if(c[0] == vert):
                  vert_o = c[1]
                  ...
                  peso = int(c[2])
                  validacion2 = 0
        if(validacion2 == 0): # Verificar si se ha encontrado vertice y arista para finalizar    
            l+=peso
            x.append(vert)
            F.append([l, vert])
            vert=vert_o
        contador+=1
        y=[]
      print("Los nodos que fueron finalizados son: ")
      print(x)
      print(F)  
